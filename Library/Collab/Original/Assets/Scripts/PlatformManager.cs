...
public class PlatformManager : PlatformGenericSinglton<PlatformManager>
{
    ...
    void Update()
    {
        ...
        if (SimulateTest)
        {
            if (i < configurationData.M)
            {
                ...
                {
                    //For displaying current tile in console
                    string s = string.Format("{0},{1}", i, j);
                    Debug.Log(s);

                    //Change position of platform[i,j] to what is now at platformData[i,j]
                    platform[i, j].GetComponent<PlatformDataNode>().NextPosition = platformData[i, j].NextPosition;

                    //Not a simulating tile
                    ...
                    //Increment j to go to next index in row
                    j++;
                }
                if (j == configurationData.N)
                {
                    //end of columns reached, increment row 
                    i++;
                    j = 0;
                }
            }
            ...
            {
                //Rows and columns have been exhausted 
                //Now,  update positions then restart indexes at 0
    ...
    private void SceneManager_sceneLoaded(Scene arg0, LoadSceneMode arg1)
    {
        switch (SceneManager.GetActiveScene().name)
        {
            ...
            case "Simulate Scene":
                ...
                //will prioritize what's in file over what has been built
                //if nothing new has been programmed
                ReadFile();
                BuildPlatform();
                break;
        ...
        //maybe add if platform != null here too
    ...
    {
        //get rid of tiles so we only have 1 set of them
        DestroyPlatform();

        //update configuration data to match input
